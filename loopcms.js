...
// Admin UI markup lives in admin.html so the source is real, parseable HTML
// instead of an escape-prone backtick template. At build time esbuild inlines
// the file via the text loader, so dist/loopcms.js remains a single file with
// no runtime fs lookup. At dev time Node can't require a .html file, so the
// catch branch reads it from disk next to loopcms.js.
let ADMIN_HTML;
try { ADMIN_HTML = require('./admin.html'); }
