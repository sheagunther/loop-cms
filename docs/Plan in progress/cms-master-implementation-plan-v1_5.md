# Loop MMT CMS — Master Implementation Plan v1.5
...
This document consolidates three CMS architectural plans, four Parallax passes, one Super RCR, one lens-RCR, one security-focused RCR, and one L21 cross-pollination RCR into a single implementation sequence:
...
- **Security RCR** (adversarial analysis — 26 findings across authentication, transport, operational security, content safety, threat modeling, and advanced threats)
- **L21 Cross-Pollination RCR** (12 architectural isomorphisms between L21 flow computer and CMS plan — config-driven contracts, request pipeline, state shape factories, single-file Tier 1, dual-format proof logging, dirty-flag verification)

The plan builds a CMS. The CMS has an engine (the capability model), a dashboard (the admin UI), a supply chain (the deployment tool), a receiving dock (the admission gate), a security system (Proof Mode), an international food aisle (localization), a checkout counter (the content editor's five buttons), produce (media), wheelchair ramps (accessibility), an undo button (Constellation Rewind), a membership card scanner (authentication), staff badges (RBAC), a food safety inspector (content sanitization), a fire marshal (rate limiting), an armored car (transport security), a safe in the back office (secret management), an FDA supply chain (dependency scanning), a one-way turnstile (the security ratchet), and a loss prevention team (intrusion detection). The grocery store metaphor carries it all, and every aisle earned its place.
...
**What this plan replaces.** The three source documents remain as detailed specifications. CMS Master Implementation Plan v1.4 is superseded by this document. This plan sequences them, resolves their overlaps, integrates 77 total findings from eight review passes plus 12 L21 isomorphisms, and provides the build order. For the mechanism behind any specific feature, read the source document.
...
**Threat model (Security RCR).** STRIDE analysis on six primary data flows, documented in Section 0 of this plan. The threat model is a Phase 0 deliverable because it governs every subsequent phase. Every security mechanism traces to a named threat, a named actor, and a named data flow. No mechanism without a threat. No threat without a mitigation or a scope exclusion.
...
**Output encoding (FBD-OE1).** Content sanitization catches input. Output encoding catches output. Even sanitized content rendered into HTML needs context-aware escaping — HTML context, attribute context, JavaScript context, URL context, CSS context. The Presentation Loop applies output encoding at render time. Sanitization at input + encoding at output = defense in depth. Neither alone is sufficient. Both together handle the case where a sanitizer bypass is discovered: the output encoding catches what the input sanitizer missed, and vice versa.
...
- Request pipeline architecture: `processRequest` six-phase function with compile-time ordering
- One-way content flow: Presentation Loops read only; admin UI dispatches to pipeline
- Running demo: two-surface CMS (public/editorial) with authenticated admin UI, RBAC (viewer/editor/publisher/admin), content creation with sanitization, media upload with ingress validation, image transforms with budget, Contentful import with sanitization, publishing workflow with role-gated transitions, draft/published isolation, locale variants (if Model A), embed support, WCAG 2.2 AA, CSRF protection, TLS, and rate limiting. **Single-file Tier 1 demo:** download one file, run one command, same spec deploys to Docker.

**Verification gate:** The Walkman test, security edition — can an editor who has never seen the system: (1) log in with their credentials, (2) see only the actions their role permits, (3) create an article, (4) upload a photo, (5) preview it on the public surface, (6) schedule publication, and (7) see it go live — without ever encountering the words "loop," "contract," "surface," "bus," or "constellation"? Then the security tests: (8) Attempt login with wrong credentials — rejected, rate-limited after 5 failures. (9) Attempt to publish without `content:publish` permission — rejected with clear message. (10) Submit content with `<script>alert('xss')</script>` in title — sanitized, script stripped. (11) Render the sanitized content on public surface — no XSS. (12) From the public Presentation Loop, attempt to query draft content — absent. (13) Upload an image — EXIF stripped. (14) Upload an SVG with `<script>` — sanitized. (15) Insert a YouTube embed — sandboxed with captions accessible. (16) Insert an unlisted embed provider — rejected. (17) Request custom media transform without budget — rejected. (18) Run `loopctl import` with Contentful export containing XSS in content fields — sanitized. (19) Inspect generated Docker Compose — no literal secrets, TLS configured. (20) Send a state-modifying request without CSRF token — rejected. (21) Check the Verification Log — startup check recorded, hash chain valid. (22) Check the deployment — non-Vault loops have ephemeral storage. (23) Run keyboard-only navigation through the admin UI — all functions reachable. (24) Run a screen reader through the admin UI — all content announced. (25) View SBOM — all dependencies listed.

**Kira:** The demo conversation now: "Your editor logged in, saw exactly what their role permits, wrote an article, and published it in three clicks. The public site didn't filter out the draft — it didn't know it existed. When someone tried to inject a script into a headline, the sanitizer stripped it before it touched the database. The photo was scrubbed of metadata. The YouTube embed is sandboxed. Every secret is in a vault. The transport is encrypted. The supply chain is audited. Migrate from Contentful in one command — and the imported content goes through the same security pipeline as everything else. The audit log is already running, tamper-evident, and the compliance evidence is one API call."

**What this phase proves:** Contract-vocabulary-as-boundary works for text and media. The admin UI proves Content Surfaces are sufficient to build editing experiences. The publishing workflow proves Bus Workflows extend to content state. Authentication and RBAC prove that the capability model extends cleanly to human users. Content sanitization proves the ingress philosophy (catch it at the input) extends from media to content. An editor can use the system without understanding the system. The operational story and the security story start on day one.
...
**Verification budgeting (FBD-VB1).** Self-verification has a compute budget: maximum CPU-seconds per verification cycle, configurable in the constellation spec. When the budget is exhausted, remaining checks are queued for the next cycle. Rapid spec changes do not trigger immediate full re-verification — they queue delta checks. Full verification runs on its schedule. An attacker who rapidly modifies specs to force verification storms gets: queued checks, rate-limited spec changes (FBD-RL1 covers the admin API), and an alert for "rapid spec changes exceeding threshold." The verification system verifies the system. It doesn't let itself be used to deny service to the system.
...
- Backup hardening: encryption, integrity verification, access control
- Running demo: multi-tenant CMS with admin UI, content surfaces, tenant-scoped media, incremental spec operations, compliance-ready proof, intrusion detection, and encrypted backups
...
**Tier boundary documentation (Security RCR).** Graham's finding, refined: each tier explicitly documents what it protects against and what it doesn't.

| Tier | Isolation Mechanism | Protects Against | Does Not Protect Against |
|------|---------------------|-----------------|------------------------|
| 1 | Scope chain (JavaScript) | Accidental cross-extension variable access | Malicious code in the same process. Tier 1 is for self-authored extensions in single-operator deployments. Untrusted extensions at Tier 1 are unsupported. |
| 2 | iframe sandbox + CSP | DOM access, cookie theft, most XSS between extension and host | Container escape CVEs, shared-kernel attacks. Docker is not a security boundary — it's a significant speed bump. |
| 3 | Container isolation (separate process, network namespace, resource limits) | Process-level data access, network-level lateral movement | Kernel exploits, hardware side channels. Residual risk is the host OS. |

This table ships in the security documentation. It is the bounded-claims principle applied to isolation tiers. "Water resistant to IPX4" not "waterproof."
...
**Factory-driven rewind (L21 Cross-Pollination — Isomorphism 7).** `captureConstellation` and `restoreConstellation` both use state shape factories. Adding a spec feature requires updating both functions. The factories are the common ground. L21 precedent: "captureFullSnapshot() serializes L21. restoreFromSnapshot(snap) writes it back. Both must be updated when L21 state changes." This has broken three times in L21 — the factories prevent the failure mode by making the shape the single source of truth.
...
|-------|------|----------------------|--------|----------|
| 0 | Foundation | Threat model, authentication architecture, RBAC model, transport security, spec shape model, contract config model, Tier 1 single-file architecture | S | Spec, docs, DX spec, threat model, auth architecture |
| 1 | Surfaces | Content sanitization, rate limiting, CSRF, session security, secret management, supply chain baseline, import sanitization, custom transform budgeting, dev/prod parity, incident response pipeline, request pipeline architecture, one-way content flow, dual-format proof logging | XL | Running CMS an editor can log into and use securely |
| 2 | Walls | Intrusion detection, backup hardening, verification budgeting, revocation propagation, tenant-scoped RBAC | L | Enterprise MVP with security monitoring |
...
**Renata:** The cumulative pitch: "A CMS where editors log in with MFA and publish in three clicks, every piece of content is sanitized before it touches the database, the API shapes what each consumer can ask about content and media, tenant isolation is proven at the database level with immediate revocation, intrusion detection watches for anomalies, extensions and AI operate in a structural sandbox with declared egress policies and bounded liability, media is stripped clean and transforms are budgeted, embeds are sandboxed with captions preserved, every secret is in a vault and rotated on schedule, transport is encrypted everywhere, the supply chain is audited, the compliance evidence is one API call, the architecture has an undo button that can't undo security, every interface meets WCAG 2.2 AA, the forensic trail is tamper-evident, you can migrate from Contentful in one command — through the same security pipeline as everything else — and you can set it up on your laptop in sixty seconds with one file and one command, with the same spec deploying to Docker or Kubernetes when you're ready."

---

## Section 6 — Cross-Cutting Concerns

Eight concerns. Each stated once.
...
**Ed:** Twenty-three risks. Each real. None fatal. Six new from the security RCR. Three new from L21 cross-pollination.
...
**FBD (Floor).** Twenty-five named controls. The original seven plus thirteen from the security RCR. FBD controls are implemented as properties of `CONTRACT_CONFIGS` entries, enforced by the `processRequest` pipeline. The config table is the FBD mechanism — adding a contract without specifying its security properties is structurally impossible because the factory requires them. Structural FBD — the code shape prevents the error. FBD-MD1 (media ingress), FBD-MV1 (dual-channel manifest verification), FBD-MG1 (translator validates against deployment tool), FBD-DX1 (reference extensions are integration tests), FBD-OP1 (deployment includes monitoring config), FBD-AC1 (admin UI WCAG 2.2 AA gate), FBD-UI1 (admin UI scope bounded by surface contracts), FBD-AU1 (authentication required for capability-bearing requests), FBD-RB1 (RBAC — every action requires named permission), FBD-CS1 (content sanitization at ingestion), FBD-OE1 (output encoding at render), FBD-CF1 (CSRF protection on state-modifying requests), FBD-TL1 (TLS required for non-localhost), FBD-SM1 (secrets as vault references, never literals), FBD-SI1 (spec output sanitization — format-appropriate escaping), FBD-RL1 (rate limiting at four layers), FBD-TB1 (custom transform budgeting), FBD-IS1 (import sanitization — same pipeline as editor ingress), FBD-SC1 (supply chain — dependency scanning blocks release on CRITICAL/HIGH), FBD-BK1 (backup encryption mandatory), FBD-LI1 (Proof Vault hash chain + application log integrity tokens), FBD-SR1 (security ratchet — irrevocable security transitions), FBD-VB1 (verification budgeting — compute cap on self-verification), FBD-BM1 (behavioral monitoring — egress to undeclared destinations blocked at Tier 2+), FBD-DP1 (dev/prod security parity — capabilities and sanitization identical). The floor is structural enforcement at every layer, for text and media, from the membership card scanner through the one-way turnstile.

**FWW(C) (Ceiling).** The grocery store now has a membership card scanner, staff badges, a food safety inspector, a fire marshal, an armored car, a safe in the back office, an FDA supply chain, a one-way turnstile, and a loss prevention team. Margaux's phase names carry the story. The demos are visceral — an editor logs in, publishes in three clicks, and doesn't know the architecture *or* the security exists. "Download one file. Run one command. You have a CMS." The single-file demo is FWW(C) at its purest. The script injection that got stripped. The wrong-credentials rejection. The role-gated buttons. The ratchet refusal. The EXIF-stripped photo. The sandboxed YouTube embed. The two-minute local dev setup. The Walkman has five buttons, plays in the rain, and you need your membership card to buy it.

**STP (Show the Path).** Three source documents, three Parallax passes, one Super RCR with Super Frame, one lens RCR, one security RCR, one L21 cross-pollination RCR. Seventy-seven findings across eight review passes plus twelve architectural isomorphisms from a working 18,900-line codebase with 331 versions. Every finding has a disposition. Every phase traces to dependencies, verification gates, and demo scripts. Twenty-three risks with mitigations. Tier boundary documentation. Two-column disclosure. Full attribution. The path from "these three documents overlap" to "here's the build order for a CMS that handles content, media, embeds, localization, accessibility, multi-tenancy, extensions, AI, compliance, authentication, RBAC, content sanitization, rate limiting, intrusion detection, supply chain, and forensic evidence — with an editor UI, an undo button, a security ratchet, and a single-file demo that runs in sixty seconds" is documented at every step.

**SNR (Signal-to-Noise).** The plan sequences. The source documents specify. Media stated once architecturally, threaded by reference. Security stated once as a threading dimension, phased by dependency. The threat model carries the structure. Each FBD control is stated once and referenced by tag. Localization stated once as a dimension. The admin UI stated once as an architectural proof. Accessibility stated once as a gate.
...
**Total: 77 findings across 8 review passes + 12 L21 isomorphisms. 77 incorporated. 12 isomorphisms integrated. 0 outstanding.**

---

[PULSE] Master CMS Implementation Plan v1.5: Three Parallax passes (12 angles, 25 findings). One Super RCR (9 items, 6 media findings). One Lens RCR (3 lenses, 8 resolutions). One Security RCR (26 findings). One L21 Cross-Pollination RCR (12 isomorphisms, 22 integration points). 77 total findings + 12 isomorphisms, all incorporated. 11 workstreams. 6 phases. 23 risks. 8 cross-cutting concerns. 25 FBD controls. Threat model (STRIDE on 6 data flows, 7 actor types). CONTRACT_CONFIGS + processRequest pipeline. State shape factories. Single-file Tier 1. Dual-format proof logging. Dirty-flag delta verification. Tier portability. Config-driven architecture collapses threading dimensions into contract properties.

[DRIVES: Floor, Ceiling, Depth, Mesh, Ground, Equalization, Constraint — 7/7]

---

*Loop MMT™ · Multi-Module Theory · CMS Master Implementation Plan v1.5*
*Three Parallax passes. One Super RCR with Super Frame. One Lens RCR. One Security RCR. One L21 Cross-Pollination RCR.*
...
*77 findings + 12 isomorphisms. All incorporated. The grocery store has a security team and a single-file demo.*
