# Loop MMT CMS — Master Implementation Plan v1.4
...
This document consolidates three CMS architectural plans, four Parallax passes, one Super RCR, one lens-RCR, and one security-focused RCR into a single implementation sequence:
...
**What this plan replaces.** The three source documents remain as detailed specifications. CMS Master Implementation Plan v1.3 is superseded by this document. This plan sequences them, resolves their overlaps, integrates 65 total findings from seven review passes, and provides the build order. For the mechanism behind any specific feature, read the source document.
...
| W11 | Security Infrastructure | Security RCR | Threading dimension (Phases 0–5); authn, RBAC, transport, secrets, rate limiting, supply chain, incident response, intrusion detection |
...
- Proof Vault + Verification Log + independent manifest verifier + scope declaration + hash chain (FBD-LI1)
- Migration tooling: `loopctl import` (with FBD-IS1 sanitization) + `loopctl translate`
...
- Running demo: two-surface CMS (public/editorial) with authenticated admin UI, RBAC (viewer/editor/publisher/admin), content creation with sanitization, media upload with ingress validation, image transforms with budget, Contentful import with sanitization, publishing workflow with role-gated transitions, draft/published isolation, locale variants (if Model A), embed support, WCAG 2.2 AA, CSRF protection, TLS, and rate limiting
...
|-------|------|----------------------|--------|----------|
| 0 | Foundation | Threat model, authentication architecture, RBAC model, transport security | S | Spec, docs, DX spec, threat model, auth architecture |
| 1 | Surfaces | Content sanitization, rate limiting, CSRF, session security, secret management, supply chain baseline, import sanitization, custom transform budgeting, dev/prod parity, incident response pipeline | XL | Running CMS an editor can log into and use securely |
...
|-------|------|-------------------|
| 1 | Editor logs in, writes article, uploads photo, previews, hits Publish. Content appears. Draft never visible. EXIF stripped. YouTube sandboxed. Script in headline sanitized. Wrong credentials rejected. No-publish user can't publish. TLS everywhere. Migrate from Contentful — sanitized. | "Your editor logged in with MFA, saw exactly what their role permits, and published in three clicks. The script someone injected was stripped before it touched the database. Migrate from Contentful in one command — through the same security pipeline." |
...
**Ed:** Twenty risks. Each real. None fatal. Six new from the security RCR.
...
**Total: 65 findings across 7 review passes. 65 incorporated. 0 outstanding.**

---

[PULSE] Master CMS Implementation Plan v1.4: Three Parallax passes (12 angles, 25 findings). One Super RCR (9 items, 6 media findings). One Lens RCR (3 lenses, 8 resolutions). One Security RCR (26 findings). 65 total findings, 65 incorporated. 11 workstreams. 6 phases. 20 risks. 6 cross-cutting concerns. 25 FBD controls. Threat model (STRIDE on 6 data flows, 7 actor types). Authentication, RBAC, content sanitization, rate limiting, transport security, secret management, supply chain, intrusion detection, behavioral monitoring, security ratchet, verification budgeting, log integrity, backup encryption, dev/prod parity, import sanitization, CSRF, incident response pipeline — all integrated.

[DRIVES: Floor, Ceiling, Depth, Mesh, Ground, Equalization, Constraint — 7/7]

---

*Loop MMT™ · Multi-Module Theory · CMS Master Implementation Plan v1.4*
*Three Parallax passes. One Super RCR with Super Frame. One Lens RCR. One Security RCR.*
*Lenses: Rugged Sony Walkman · F1 Racing Operations · McMaster-Carr*
*65 findings. 65 incorporated. The grocery store has a security team.*
