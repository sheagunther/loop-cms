# Loop MMT CMS — Master Implementation Plan v1.7
...
This document consolidates three CMS architectural plans and ten review passes into a single implementation sequence:
...
**What this plan replaces.** CMS Master Implementation Plan v1.6 is superseded. This plan integrates 104 total findings from ten review passes plus 12 L21 isomorphisms. For mechanism detail, read the source documents.
...
- Publishing Workflow Pipeline declaration
...
- Admin UI: Presentation Loop on editorial surface, RBAC-gated, WCAG 2.2 AA (FBD-AC1), CSRF-protected (FBD-CF1), with first-run experience
- Deployment tool v1: spec → Docker Compose + single-file, dry run (FBD-DD1), Spec Version Store, TLS (FBD-TL1), secrets (FBD-SM1), spec sanitization (FBD-SI1)
- Request pipeline: `processRequest` six-phase function with bounded execution (FBD-TE1), sanitization verification (FBD-SV1), one-way content flow
...
- Running demo: single-file Tier 1 setup in sixty seconds → editor publishes in three clicks → same spec deploys to Docker. Plus full two-surface verification suite.
...
**Demo narrative (FWW(C) + Hitchhiker's RCR).** The verification gate is QA. The demo is a movie. No plugin conflicts. No database migration. No "your PHP version is incompatible." Alex — a content editor who has never seen this CMS — downloads one file, runs one command. Ninety seconds later, Alex is looking at "Hello, World — your first article" in the admin UI. Alex writes the company rebrand article, uploads the new logo (EXIF stripped without Alex knowing), copies a code snippet with a `<script>` tag from the old site (the UI shows "We cleaned up some HTML in your title — script tags aren't allowed in content"), previews on the public surface, schedules publication for Monday, and sees five buttons and nothing else. Sixty seconds if you're fast. Three minutes if you're showing it to a buyer. The architecture is invisible. The experience is the product.
...
|-------|------|----------------------|--------|----------|
| 0 | Foundation | Threat model, authentication, RBAC, transport security, spec shape model, contract config model, Tier 1 single-file, **system lifecycle state machine**, system voice, commercial architecture | M | Spec, docs, DX spec, threat model, auth architecture, lifecycle spec, voice guide. Most consequential review surface in the plan — density is high. |
...
|-------|------|-------------------|-------------|
| 1 | **Download one file. Run one command. Sixty seconds later, Alex publishes an article.** No plugin conflicts. No database migration. EXIF stripped. Script sanitized (Alex sees "we cleaned that up"). Same spec deploys to Docker. Migrate from Contentful in one command. | "Set up a CMS on your laptop in sixty seconds. When you're ready for production, the same spec deploys to Docker, Kubernetes, any cloud." | "I can see the app. It works. It took a minute to set up." |
| 2 | Multi-tenant with Proof Endpoint, intrusion detection, encrypted backups, SOC 2 evidence. Revoke an extension — instant. Staging twin validates before production. | "Tenant isolation at the database level. SOC 2 evidence in one API call. Staging twin catches problems before production." | "Multiple clients, each seeing only their stuff. The compliance paperwork is automatic." |
...
**Ed:** Twenty-five risks. Each real. None fatal.
...
**L21 integration note.** FBD-CS1, FBD-RL1, FBD-RB1, FBD-LI1, FBD-TB1, and FBD-MD1 are implemented as properties of `CONTRACT_CONFIGS` entries, enforced by the `processRequest` pipeline.

**Phase 1 — 26 controls (prevention + fault response):**
...
**Total: 104 findings across 10 review passes + 12 L21 isomorphisms. All incorporated. 0 outstanding.**

---

[PULSE] Master CMS Implementation Plan v1.7: Ten review passes. 104 findings + 12 isomorphisms, all incorporated. 30 FBD controls grouped by phase. System Voice (CLI + errors + Proof Vault — one voice, three surfaces). First-Run Experience (ninety seconds to first article). Demo Narrative (Alex's story — familiar pain, then novel solution). Jamie's View (every phase explained without architectural vocabulary). Commercial Architecture (pricing, buyer, landing page, positioning — SEP field cleared). Phase 0 re-estimated S→M (bistromathics). Single-file as product lead, not tier variant. Grocery store metaphor released from strained duty. The engineering is the track. The FWW(C) is the reason you put your hands up.

[DRIVES: Floor, Ceiling, Depth, Mesh, Ground, Equalization, Constraint — 7/7]

---

*Loop MMT™ · Multi-Module Theory · CMS Master Implementation Plan v1.7*
*Ten review passes. 104 findings + 12 isomorphisms. All incorporated.*
*Lenses: Rugged Sony Walkman · F1 Racing Operations · McMaster-Carr · NASA Flight Software Engineering · Hitchhiker's Guide to Problem-Solving*
*Isomorphisms: L21 flow computer (v.331, 18,900 lines, 574 functions)*
*Download one file. Run one command. You have a CMS. The rest is upgrade path.*
